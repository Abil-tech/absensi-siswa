  ...
  },
  {
    ...
    icon: (active: boolean) => (
      <svg width="19" height="19" viewBox="0 0 20 20" fill="none">
        ...
        <path d="M10 7v3M10 12.5h.01" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" opacity={active ? "1" : ".7"} />
      </svg>
    ),
  },
  {
...
export default function BkSidebar({ open, onClose }: SidebarProps) {
    ...
    `}>
      ...
      <div className="px-6 pt-8 pb-6 border-b border-white/10">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-white/40 text-[11px] font-medium mt-0.5">Guru BK</p>
          </div>
        </div>
      </div>
            ...
            >
              {active && (
                isMobile ? (
                  ...
                  <motion.span
                    layoutId="bk-sidebar-pill"
                    className="absolute inset-0 rounded-xl bg-[#7fe05b]"
                    transition={{ type: "spring", stiffness: 380, damping: 34 }}
                  />
                )
              )}
              <span className={`relative z-10 ${active ? "text-[#111410]" : "text-white/40 group-hover:text-white/70"}`}>
                {item.icon(active)}
              </span>
              <span className={`relative z-10 text-[13px] ${active ? "text-[#111410]" : "text-white/50 group-hover:text-white"}`}>
                {item.label}
              </span>
            </Link>
          );
      ...
      <div className="px-4 pb-7 pt-4 border-t border-white/10">
        <div className="flex items-center gap-3 px-2 py-2">
          <div className="w-10 h-10 rounded-full bg-[#7fe05b] flex items-center justify-center text-[#111410] font-black text-sm shrink-0">BK</div>
          <div className="flex-1 min-w-0">
            <p className="text-white text-[13.5px] font-bold truncate">Counselor Smith</p>
            <p className="text-white/40 text-[11px] truncate">Guru BK</p>
          </div>
          <button
