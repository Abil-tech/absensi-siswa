...
interface SiswaData {
  ...
  status: StatusAbsensi; waktu: string;
}

interface KelasData {
  id: string; nama: string; waliKelas: string | null;
  total: number; hadir: number; terlambat: number;
  sakit: number; izin: number; belumAbsen: number;
  siswa: SiswaData[];
}
...
interface DispensasiItem {
  ...
  status: StatusDispensasi; catatanBK: string | null;
  tanggal: string;
  siswa: { id: string; nama: string; nis: string; kelas: string };
}
...
function downloadSiswaExcel(detail: DetailSiswa) {
  ...
  const tanggal = formatTanggal(new Date().toISOString());
...
export default function BkDashboardContent({ onMenuClick }: Props) {
  ...
  useEffect(() => { scrollRef.current?.scrollTo(0, 0); }, [selectedKelas]);
    ...
    try {
      ...
      setDispensasiList((prev) =>
        prev.map((d) => d.id === dispensasiId
          ? { ...d, statusBK: keputusan, status: json.status } : d)
      );
  ...
  const BtnDownload = ({ onClick, label }: { onClick: () => void; label: string }) => (
    ...
    <div ref={scrollRef} className="flex-1 flex flex-col min-h-screen overflow-y-auto">
      ...
      <header className="sticky top-0 z-10 flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16 bg-[#f5f5ef]/90 backdrop-blur border-b border-black/5">
        <div className="flex items-center gap-3 min-w-0">
          <button type="button" onClick={onMenuClick} className="lg:hidden text-[#1a1a1a] p-1 shrink-0">
          ...
          {selectedKelas ? (
            <div className="flex items-center gap-2 min-w-0">
              <button type="button" onClick={() => { setSelectedKelas(null); setSearch(""); }}
                className="flex items-center gap-1.5 text-[#9a9a9a] hover:text-[#1a1a1a] transition-colors shrink-0">
              ...
              <h1 className="text-[1rem] sm:text-[1.3rem] font-extrabold text-[#1a1a1a] tracking-tight truncate">
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-5 sm:py-7 flex flex-col gap-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {kelasGroup.map((kelas) => {
                        const pct = kelas.total > 0
                          ...
                          : 0;
                        return (
                          <button key={kelas.id} type="button" onClick={() => setSelectedKelas(kelas)}
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {kelas.hadir > 0 && <span className="text-[11px] font-semibold bg-[#f0fce8] text-[#4a9e2f] px-2 py-0.5 rounded-full">{kelas.hadir} Hadir</span>}
                              {kelas.terlambat > 0 && <span className="text-[11px] font-semibold bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full">{kelas.terlambat} Terlambat</span>}
                              {kelas.sakit > 0 && <span className="text-[11px] font-semibold bg-red-50 text-red-600 px-2 py-0.5 rounded-full">{kelas.sakit} Sakit</span>}
                              {kelas.izin > 0 && <span className="text-[11px] font-semibold bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{kelas.izin} Izin</span>}
                              {kelas.belumAbsen > 0 && <span className="text-[11px] font-semibold bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full">{kelas.belumAbsen} Belum Absen</span>}
                            </div>
        ...
        {selectedKelas && (
          <>
            ...
            {/* Tabel siswa */}
            <div className="bg-white rounded-2xl shadow-[0_2px_16px_rgba(0,0,0,0.05)] overflow-hidden">
              <div className="px-4 sm:px-6 py-4 border-b border-black/5 flex items-center gap-2">
              ...
              <div className="hidden sm:block">
                ...
                <div className="divide-y divide-black/[0.04]">
                  {filteredSiswa.map((s, i) => (
                    <button key={s.id} type="button" onClick={() => handleOpenDetail(s)}
