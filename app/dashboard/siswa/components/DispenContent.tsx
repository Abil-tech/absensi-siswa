...
export default function DispenContent({ onMenuClick }: Props) {
  ...
  async function handleSubmit() {
    if (!file) {
      alert("Harap unggah dokumen dispensasi terlebih dahulu.");
      return;
    }
    setLoading(true);
    ...
    // formData.append("status", "Dispen");
    // formData.append("keterangan", keterangan);
    // formData.append("file", file);
    // await fetch("/api/dispen", { method: "POST", body: formData });
          ...
          <div className="bg-white rounded-2xl shadow-[0_2px_24px_rgba(0,0,0,0.06)]">
            ...
            {submitted ? (
              <div className="p-8 sm:p-12 flex flex-col items-center text-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-[#9a9a9a] text-sm mt-1.5">
                    Pengajuan dispensasi berhasil dicatat
                  </p>
                  <p className="text-[#b0b0a8] text-[12.5px] mt-0.5">{tanggal} · {waktu}</p>
                </div>

                {/* File info */}
                <div className="flex items-center gap-3 bg-[#f9f9f5] rounded-xl px-5 py-3.5 w-full max-w-xs">
                ...
                {keterangan && (
                  <div className="bg-[#f9f9f5] rounded-xl px-5 py-3.5 w-full max-w-xs text-left">
                    <p className="text-[11px] font-bold text-[#9a9a9a] uppercase tracking-wide mb-1">Keterangan</p>
                    <p className="text-[13px] text-[#1a1a1a] font-medium">{keterangan}</p>
                  </div>
                )}
              ...
              <div className="p-5 sm:p-8 flex flex-col gap-6">
                ...
                {/* ── Status Dispen (satu opsi) ── */}
                <div className="flex items-center gap-4 bg-[#f0fce8] border-2 border-[#7fe05b] rounded-2xl px-5 py-4">
                  <div className="w-11 h-11 rounded-xl bg-[#7fe05b] flex items-center justify-center shrink-0">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                      <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6z" stroke="#111410" strokeWidth="2" strokeLinejoin="round" />
                      <path d="M14 2v6h6M9 13h6M9 17h4" stroke="#111410" strokeWidth="1.8" strokeLinecap="round" />
                    </svg>
                  </div>
                  <div className="flex-1">
                    <p className="text-[14px] font-extrabold text-[#111410]">Dispensasi</p>
                    <p className="text-[12px] text-[#4a9e2f] font-medium mt-0.5">
                      Pengajuan izin keluar / meninggalkan kelas
                    </p>
                  </div>
                  <span className="text-[11px] font-black text-[#4a9e2f] bg-[#7fe05b]/30 px-2.5 py-1 rounded-full shrink-0">
                    Aktif
                  </span>
                </div>
                    ...
                    <div className="relative rounded-2xl overflow-hidden border-2 border-[#7fe05b]">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-black/50 px-4 py-2.5 flex items-center justify-between">
                        ...
                        <button
                          type="button"
                          onClick={removeFile}
                          className="text-white/70 hover:text-white shrink-0 ml-2 p-0.5"
                      ...
                      <button
                        ...
                        className="absolute top-3 right-3 bg-white/90 hover:bg-white text-[#111410] text-[11.5px] font-bold px-3 py-1.5 rounded-full transition active:opacity-70"
                      >
                        Ganti File
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className={`
                        ...
                        ${dragOver
                          ...
                          : "border-[#d0d0c8] bg-[#fafaf7] hover:border-[#7fe05b] hover:bg-[#f0fce8] active:bg-[#f0fce8]"
                        }
                      `}
                    >
                      <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-colors ${dragOver ? "bg-[#7fe05b]/20" : "bg-[#f0fce8]"}`}>
                        <svg width="30" height="30" viewBox="0 0 32 32" fill="none" className="text-[#7fe05b]">
                          ...
                          <rect x="4" y="4" width="24" height="24" rx="4" stroke="currentColor" strokeWidth="1.4" opacity=".15" />
                        </svg>
                      </div>
                      <div>
                        <p className="text-[13.5px] font-bold text-[#1a1a1a]">
                          {dragOver ? "Lepaskan file di sini" : "Unggah Dokumen Dispensasi"}
                        </p>
                        <p className="text-[12px] text-[#9a9a9a] mt-1">
                          Klik untuk memilih file
                        </p>
                        <p className="text-[11.5px] text-[#b0b0a8] mt-0.5">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-bold text-[#9a9a9a] uppercase tracking-wide">
                    Keterangan <span className="normal-case font-normal text-[#c0c0b8]">(opsional)</span>
                  </label>
                  <textarea
                    rows={3}
                    value={keterangan}
                ...
                {!file && (
                  <p className="text-center text-[12px] text-[#b0b0a8]">
                    * Unggah dokumen dispensasi terlebih dahulu sebelum mengirim
                  </p>
                )}
