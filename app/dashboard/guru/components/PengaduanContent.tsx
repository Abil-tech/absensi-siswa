...
const STORAGE_KEY = "pengaduan_aktif_id";
...
export default function PengaduanContent({ onMenuClick }: Props) {
  ...
  // view diinisialisasi "form" dulu; akan diupdate di useEffect
  // setelah cek localStorage (hindari hydration mismatch)
  const [view, setView]                     = useState<"form" | "chat">("form");
  const [activePengaduan, setActivePengaduan] = useState<PengaduanData | null>(null);
  const [restoring, setRestoring]           = useState(true); // true saat sedang restore session
  ...
  // ── Helper: simpan / hapus pengaduanId dari localStorage ──
  function persistPengaduanId(id: string | null) {
    if (id) localStorage.setItem(STORAGE_KEY, id);
    else     localStorage.removeItem(STORAGE_KEY);
  }

  // ── Fetch satu pengaduan berdasarkan id ──
  const fetchPengaduanById = useCallback(async (id: string): Promise<PengaduanData | null> => {
    try {
      const res  = await fetch("/api/pengaduan");
      if (!res.ok) return null;
      const json = await res.json();
      return json.pengaduan.find((p: PengaduanData) => p.id === id) ?? null;
    } catch {
      return null;
    }
  }, []);
  ...
  useEffect(() => {
    async function restore() {
      ...
      const pengaduan = await fetchPengaduanById(savedId);

      if (!pengaduan) {
        // Pengaduan tidak ditemukan di server — hapus dari storage
        persistPengaduanId(null);
        setRestoring(false);
        return;
      }

      if (pengaduan.status === "open") {
        // Masih aktif → langsung ke chat
        setActivePengaduan(pengaduan);
        setView("chat");
      } else {
        // Sudah selesai/ditutup → bersihkan storage, tampilkan form
        persistPengaduanId(null);
      }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const updated = await fetchPengaduanById(activePengaduan.id);
      if (updated) setActivePengaduan(updated);
    }, 5000);
    return () => clearInterval(interval);
  }, [view, activePengaduan?.id, fetchPengaduanById]);
  ...
  async function handleSubmitForm() {
    ...
    try {
      ...
      // Simpan id ke localStorage sebelum ganti view
      persistPengaduanId(json.id);

      const pengaduan = await fetchPengaduanById(json.id);
      if (pengaduan) setActivePengaduan(pengaduan);
  ...
  async function handleKirimPesan() {
    ...
    try {
      ...
      setActivePengaduan((prev) =>
        prev
          ? { ...prev, messages: [...prev.messages, { role: "walas", pesan: pesanBaru, createdAt: new Date().toISOString() }] }
          : prev
      );
      setPesanBaru("");
    } catch {
      setChatError("Gagal terhubung ke server.");
    } finally {
  ...
  async function handleUpdateStatus(keputusan: "selesai" | "tidak_selesai") {
    ...
    try {
      ...
      setActivePengaduan((prev) =>
        prev ? { ...prev, status: json.status, tidakSelesaiCount: json.tidakSelesaiCount } : prev
      );

      // Pengaduan selesai atau ditutup → bersihkan storage lalu kembali ke form
  ...
  // ── Loading saat restore session ──
  if (restoring) {
    return (
      <div className="flex-1 flex items-center justify-center min-h-screen">
        <div className="flex flex-col items-center gap-3">
          <span className="w-8 h-8 border-[3px] border-[#e8e8e0] border-t-[#7fe05b] rounded-full animate-spin" />
          <p className="text-[13px] text-[#9a9a9a] font-semibold">Memuat pengaduan...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 flex flex-col min-h-screen overflow-y-auto">
        ...
        <main className="flex-1 px-4 sm:px-6 lg:px-8 py-5 flex flex-col gap-4 max-w-2xl mx-auto w-full">
          ...
          {!isClosed && activePengaduan.messages.some((m) => m.role === "bk") && (
          ...
          {isClosed && (
            <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-center text-[13px] text-[#9a9a9a] font-medium">
              {activePengaduan.status === "selesai"
                ? "Pengaduan telah diselesaikan."
                : "Pengaduan ditutup setelah 3x tidak selesai."}
            </div>
          )}
        </main>
      </div>
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-2xl shadow-[0_2px_24px_rgba(0,0,0,0.06)] p-5 sm:p-8 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <div
                  ...
                  className={`flex items-center gap-3 px-4 py-3 cursor-pointer rounded-xl border transition-all duration-200 ${showDropdown ? "border-[#7fe05b] bg-white" : "border-transparent bg-[#f0f0ea]"}`}
                >
                  {loadingSiswa ? (
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1.5 bg-white rounded-xl border border-black/10 shadow-xl z-20 overflow-hidden">
                    <div className="p-2 border-b border-black/5">
                      <input
                        ...
                        className="w-full pl-3 pr-3 py-2 text-[13px] bg-[#f0f0ea] rounded-lg outline-none focus:bg-white border border-transparent focus:border-[#7fe05b] transition-all"
                      />
                    </div>
                    <div className="max-h-48 overflow-y-auto">
                      {filteredSiswa.length === 0 ? (
                        ...
                        >
                          <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-[10px] font-black shrink-0" style={{ background: AVATAR_COLORS[i % AVATAR_COLORS.length] }}>
                            {getInitials(s.nama)}
                          </div>
                          <div>
                ...
                {KATEGORI_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 text-left transition-all duration-150 ${active ? "border-[#7fe05b] bg-[#f0fce8]" : "border-[#e8e8e0] bg-[#f9f9f5]"}`}
                    >
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-bold text-[#9a9a9a] uppercase tracking-wide">
                Deskripsi / Pesan Awal <span className="text-red-400">*</span>
              </label>
              <textarea
                rows={4}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {file ? (
                ...
                <button
                  ...
                  className="flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 border-dashed border-[#d0d0c8] bg-[#fafaf7] hover:border-[#7fe05b] hover:bg-[#f0fce8] transition-all duration-200"
                >
            ...
            <button
              ...
              className="w-full py-4 rounded-2xl bg-[#111410] hover:bg-[#1e1e16] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2.5 transition-all duration-150"
            >
              {formLoading ? (
